...
`event`、`state_snapshot` 和 `bye` frames，然后退出。`RuntimeFrame` 保留 `log`
frames 给显式转发日志的 hosts 或 transports；worker entrypoint 不会把 stdout 或 stderr
解析成 control frames。Long-lived worker pools、queue loops 和 Docker clients 属于 SDK 外部。
