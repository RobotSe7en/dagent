...
| `tool.write_file` | medium | Write UTF-8 text to a file, creating parent directories. New files follow the process umask, replacement writes preserve the existing file mode, and hardlinked files are rewritten in place so sibling links see the new content. Returns the byte count written. |
