...
| `tool.list_files` | low | List files and directories under a path (directories end with `/`), up to `depth` levels (default 3). With `glob` (e.g. `*.py`) it lists matching files only. Output is capped at 500 entries; the structured result value is the entry list, so DAG map nodes can fan out over it. |
