...
test('updated orchestration and tools workspaces use real backend data with the design shell', async () => {
  ...
  assert.match(orchestrationSource, /<CanvasViewportControls flowInstance=\{flowInstance\} hasNodes=\{nodes\.length > 0\} \/>/);
  ...
  assert.match(appSource, /flowInstance\.fitView\(\{ padding: 0\.25, duration: 220 \}\)/);
  ...
  assert.match(dynamicSource, /<CanvasViewportControls flowInstance=\{flowInstance\} hasNodes=\{nodes\.length > 0\} \/>/);
