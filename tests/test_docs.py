...
def test_migration_notes_keep_unreleased_persistence_changes_out_of_release_history() -> None:
    english = Path("docs/en/migration.md").read_text(encoding="utf-8")
    chinese = Path("docs/zh-CN/migration.md").read_text(encoding="utf-8")

    english_unreleased = _section(english, "## Unreleased", "## 0.6.3")
    english_063 = _section(english, "## 0.6.3", "## 0.6.2")
    english_released = _section(english, "## 0.6.1", "## 0.6.0")
    chinese_unreleased = _section(chinese, "## Unreleased", "## 0.6.3")
    chinese_063 = _section(chinese, "## 0.6.3", "## 0.6.2")
    chinese_released = _section(chinese, "## 0.6.1", "## 0.6.0")

    assert "local API/WebUI store now isolates chat" in english_unreleased
    assert "Incompatible pre-release local SQLite API databases are recreated" in english_unreleased
    ...
    assert "本地 API/WebUI store 现在按 kind 隔离普通 chat" in chinese_unreleased
    assert "不兼容的未发布本地 SQLite API 旧库时会直接重建数据库" in chinese_unreleased
